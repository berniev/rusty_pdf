...
fn test_objstm_has_type() {
    ...
    let color = RGB { red: Color { color: 0.0 }, green: Color { color: 0.0 }, blue: Color { color: 1.0 } };
    ...
    pdf.write_compressed(&mut output, FileIdentifierMode::None).expect("write failed");

    let pdf_str = String::from_utf8_lossy(&output);
    assert!(pdf_str.contains("/Type /ObjStm"), "Object stream must have /Type /ObjStm");
}
...
fn test_objstm_has_n() {
    ...
    if let Some(n_pos) = pdf_str.find("/N ") {
        ...
        assert!(n_str.parse::<usize>().is_ok(), "/N must be an integer, got: {}", n_str);
    }
}
...
fn test_objstm_has_first() {
    ...
    let pdf_str = String::from_utf8_lossy(&output);
    assert!(pdf_str.contains("/First "), "Object stream must have /First entry");
    ...
    if let Some(first_pos) = pdf_str.find("/First ") {
        ...
        assert!(first_str.parse::<usize>().is_ok(), "/First must be an integer, got: {}", first_str);
    }
}
...
fn test_objstm_has_filter() {
    ...
    assert!(pdf_str.contains("/Type /ObjStm"), "Object stream must have /Type /ObjStm");
    assert!(pdf_str.contains("/Filter"), "Object stream must have /Filter entry");
    assert!(pdf_str.contains("FlateDecode"), "Object stream must use FlateDecode filter");
}
...
fn test_content_streams_not_compressed() {
    ...
    assert!(pdf_str.contains("1 0 obj"), "Content stream should be written directly as object 1");
}
