...
fn test_array_object_uses_data_not_reference() {
    ...
    assert!(
        !data.contains(" R"),
        "Array should not contain indirect references"
    );
}
...
fn test_compressed_object_stream_is_valid() {
    ...
    match decoder.read_to_end(&mut decompressed) {
        Ok(_) => {
            ...
            assert!(
                decompressed_str.contains("/Type /Font"),
                "Missing Font object"
            );
            assert!(
                decompressed_str.contains("/Type /Pages"),
                "Missing Pages object"
            );
        }
        Err(e) => {
            panic!(
                "Failed to decompress: {}. Compressed length: {}",
                e,
